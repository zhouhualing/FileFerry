...
namespace WD.Library.Core
{
	...
	{
		public static List<T> BuildItems<T>(string path, object parameter = null, bool throwOnNotFound = true)
		{
				...
				if (addIn.Enabled) {
					foreach (KeyValuePair<string, Version> pair in addIn.Manifest.Identities) {
						if (dict.ContainsKey(pair.Key)) {
							GlobalService.Resolve<IMessageService>().ShowError("Name '" + pair.Key + "' is used by " +
		...
		checkDependencies:
			for (int i = 0; i < list.Count; i++) {
				...
				foreach (AddInReference reference in addIn.Manifest.Conflicts) {
					if (reference.Check(dict, out versionFound)) {
                        GlobalService.Resolve<IMessageService>().ShowError(addIn.Name + " conflicts with " + reference.ToString()
				...
				foreach (AddInReference reference in addIn.Manifest.Dependencies) {
					if (!reference.Check(dict, out versionFound)) {
						if (versionFound != null) {
                            GlobalService.Resolve<IMessageService>().ShowError(addIn.Name + " has not been loaded because it requires "
			...
			foreach (AddIn addIn in list) {
				try {
                    ...
                    //LoggingService.Error(ex);
                    GlobalService.Resolve<IMessageService>().ShowError("Error loading AddIn " + addIn.FileName + ":\n"
					                         + ex.Message);
				}
			}
		}
